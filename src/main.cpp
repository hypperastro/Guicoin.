...
#if defined(NDEBUG)
# error "GuiCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xed6f88c5f81fd6d1b8b13f1a6747d45845f25381c1a523d25c87b862ad8d11f1");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // GuiCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "GuiCoin Signed Message:\n";
...
{
    // GuiCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 694200 blocks, which will occur approximately every 2.5 years
    nSubsidy >>= (nHeight / 694200); // GuiCoin: 694200 blocks in ~2.5 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // GuiCoin: 3.5 days
static const int64 nTargetSpacing = 1.5 * 60; // GuiCoin: 1.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // GuiCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // GuiCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xa06e058aaaafd93b672f2bff19ae5e231715d9e2a0c86516e318a7c35e9b29bb");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "No intresting news for today folks 05/02/2022";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 69 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04cab711a04f45f71fda27ef861cb51092f8056871fbe2562ccc23bbfc6228bcf0c1fcc288881e9e8551dc50a54b3612197e2ce885c7acf2ae4a00ebfb264e0e02") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1644054074;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084976096;

        if (fTestNet)
        {
            block.nTime    = 1644054020;
            block.nNonce   = 385956762;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xd8b431d4bc8fca9ce8a66e8d73cd821df21511e1b35b63123936b8712d351d2c"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf4, 0xcb, 0xb5, 0xda }; // GuiCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GuiCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GuiCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GuiCoinMiner(CWallet *pwallet)
{
    printf("GuiCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("guicoin-miner");
    ...
    try { loop {
        ...
        printf("Running GuiCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GuiCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GuiCoinMiner, pwallet));
}
